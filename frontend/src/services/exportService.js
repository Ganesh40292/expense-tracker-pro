...
export const generatePDFReport = (transactions, user, chartImages = [], aiInsights = []) => {
  ...
  // Generate PDF Blob with explicit MIME type and trigger save
  const pdfBlob = doc.output('blob')
  const pdfFileName = `Expense_Report_${new Date().toISOString().slice(0, 10)}.pdf`
  saveAs(new Blob([pdfBlob], { type: 'application/pdf' }), pdfFileName)
}
