...
export default function Dashboard() {
  ...
  const handleBudgetSubmit = async (e) => {
    e.preventDefault()
    if (!budgetForm.amount || parseFloat(budgetForm.amount) <= 0) return
    try {
      await api.post('/budgets', budgetForm)
      showToast('Category budget set successfully!', 'success')
  ...
  const convertedTransactions = useMemo(() => {
    ...
    return transactions.map((t) => {
      const amount =
        t.currency?.toUpperCase() === defaultCurrency.toUpperCase()
          ? t.amount
    ...
    <div className="ag-dashboard">
      ...
      >
        <div className="ag-dashboard__header-left">
          <h1 className="ag-dashboard__title">Expense Dashboard</h1>
          <p className="ag-dashboard__subtitle">
            Welcome back, <span className="ag-highlight">{user?.name || 'User'}</span>. Here's your financial overview at a glance.
          </p>
        </div>
