...
function Transactions() {
  ...
  const { filteredTransactions, setStartDate, setEndDate, clearAllFilters } = filterState
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const formattedDescription = `[${paymentMethod}] ${form.description}`.trim()
      const payload = {
        ...form,
      ...
      // Category budget warning check
      if (form.type === 'EXPENSE') {
        try {
          const budgetsRes = await api.get('/budgets')
          const catBudget = (budgetsRes.data || []).find(b => b.category === form.category)
          if (catBudget && catBudget.amount > 0) {
            const spent = catBudget.spent || 0
            const percent = ((spent + parseFloat(form.amount)) / catBudget.amount) * 100
            if (percent >= 100) {
            ...
            return (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg border font-medium transition-all shrink-0 ${
                  isActive
                    ? 'bg-cyan-500/30 border-cyan-400 text-cyan-200 font-bold shadow-[0_0_12px_rgba(34,211,238,0.3)]'
                    : 'bg-slate-800/60 hover:bg-cyan-500/20 border-slate-700/50 hover:border-cyan-500/40 text-slate-300 hover:text-cyan-300'
                }`}
              >
