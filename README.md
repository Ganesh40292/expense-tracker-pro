...
- **Conversational & Analytics Intelligence**: Linear regression forecasting, cumulative outflow area volume charts, and z-score anomaly diagnostics.
- **Real-Time Financial Wellness Indexing**: Computes a dynamic 0–100 Financial Health Score based on savings discipline, category cap adherence, and spending volatility with a glowing semi-circle gauge.
- **Interactive Notification Center**: Real-time bell notification dropdown tracking monetary activities, unread alerts, and mark-all-read management.
...
### 📊 Expense Dashboard & Health Speed Gauge
![Expense Dashboard](docs/screenshots/dashboard.png)

### 🤖 AI Financial Intelligence & Cumulative Trends
...
### 💳 Transaction Management & Quick Filter Presets
...
2. **Linear Regression Forecast Model & Cumulative Outflow Charts**:
   ...
   - Renders **Cumulative Outflow Volume Area Charts** for multi-dimensional spending trend visualization.
3. **Z-Score Outlier Anomaly Detection**:
...
1. **Interactive Financial Wellness Speed-Gauge (0–100)**:
   - Animated SVG semi-circle gauge displaying real-time financial health score with dynamic color-coded arcs (**Red** `<50`, **Yellow** `50–79`, **Green** `80–100`).
...
4. **1-Click Executive PDF Statement Exporter**:
   - Generates styled PDF statements containing balance summaries, category tables, and AI executive briefs using `jsPDF` and `html2canvas`.
...
### 🎨 Frontend User Experience & System Enhancements
1. **Interactive Notification Bell Dropdown**:
   - Real-time notification menu in top navigation tracking financial activity, transaction logs, and mark-all-read functionality.
2. **Refined Design System & Pointy Cards**:
   - Modernized card border radius (`10px` angular corners) and increased container padding (`24px`) for clean spacing.
   - Stat cards featuring left edge color accents and elevated hover glow effects.
3. **Sidebar Help & Support Section**:
   - Dedicated Help section in sidebar with direct link to Help Center and support email (`support@expensetracker.com`).
4. **Upgraded Profile & Security Portal**:
   - Hero header banner with avatar upload, user role badge, currency indicator, and a side-by-side 2-column layout for Profile Details & Password Security.
5. **Global Command Palette (`Ctrl + K`)**:
   - Keyboard listener modal across all protected routes for instant single-keypress navigation.
6. **Multi-Currency Switcher & Formatter**:
   - Supports `INR (₹)`, `USD ($)`, `EUR (€)`, and `GBP (£)` dynamically formatted across every card, chart, and statement.
7. **Quick Date Filter Presets**:
   - Single-click filter chips for `Today`, `This Week`, `This Month`, `This Year`, or custom date ranges with active state highlighting.
 ...
 └── CurrencyProvider (CurrencyContext)
      └── Router (React Router v7)
           ...
           └── ProtectedLayout
                ...
                ├── Navbar (Avatar, Currency Switcher, Notification Bell Dropdown, Search Pill)
                ├── Sidebar (Navigation Items & Help & Support Section)
                ├── CommandPalette Modal (Ctrl + K)
                └── Route Outlets (/dashboard, /transactions, /assets, /debt, /receipts, /reports, /profile)
```
